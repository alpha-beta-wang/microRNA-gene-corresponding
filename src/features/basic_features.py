...
def compute_sequence_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for nt in ["A", "C", "G", "T"]:
        features[f"gene_{nt}_ratio"] = gene_seq.str.count(nt) / gene_seq.str.len().replace(0, np.nan)

    for nt in ["A", "C", "G", "U"]:
        features[f"mirna_{nt}_ratio"] = mirna_seq.str.count(nt) / mirna_seq.str.len().replace(0, np.nan)
